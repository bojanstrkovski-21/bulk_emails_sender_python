...
class EmailSenderGUI:
    def __init__(self, root):
        ...
        # Message Body
        tk.Label(root, text="Email Message:", font=("Arial", 10, "bold")).pack(pady=(10, 5))
        ...
        self.log_text = scrolledtext.ScrolledText(root, width=70, height=8, state=tk.DISABLED)
    ...
    def load_excel(self):
        ...
        try:
            ...
            self.file_label.config(text=f"Loaded: {os.path.basename(file_path)} ({len(self.recipients)} recipients)", fg="green")
            self.log(f"Loaded {len(self.recipients)} recipients from Excel file")
